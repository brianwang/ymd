...
- [ ] 2. 小程序端：空间详情页咨询入口落地
  - [ ] 2.1 更新空间详情页“咨询”交互：弹出咨询方式选择（表单/复制微信/拨号）
  - [ ] 2.2 实现“提交咨询”表单：字段校验、loading/错误态、提交成功反馈
  - [ ] 2.3 对接后端创建接口并传递必要字段（space_id、联系方式、留言、可选 user_id）
  - [ ] 2.4 统一配置客服微信号与电话（优先走前端配置项/环境变量；无则隐藏对应入口）

- [ ] 3. 管理后台：咨询线索管理页面
  - [ ] 3.1 新增“咨询线索”菜单与列表页（表格 + 分页 + status 筛选）
  - [ ] 3.2 支持更新线索状态与备注（对接 PATCH 接口）
  - [ ] 3.3 基础体验：空态/错误态/权限错误提示

- [ ] 4. 联调与回归
  - [ ] 4.1 后端：确认仅 superuser 可访问 admin 线索接口
  - [ ] 4.2 前端：在空间详情页完成一次完整咨询提交流程
  - [ ] 4.3 管理端：可看到新提交线索并能更新状态
