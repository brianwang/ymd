<template>
  <div class="card adm-panel">
    <div class="adm-panel-head bordered">
      <div>
        <div class="adm-hint">支持筛选与删除（会回写帖子的评论数）</div>
        <div class="adm-filters">
          ...
          <input v-model="postIdRaw" class="input f sm" placeholder="帖子ID" @keyup.enter="applyFilters" />
      ...
      <div class="adm-actions">
        ...
        <button class="btn" :disabled="loading || comments.length < limit" @click="next">下一页</button>
      </div>
    </div>
    ...
    <div v-else class="adm-table-wrap">
      ...
      <tbody>
        <tr v-for="c in comments" :key="c.id">
          ...
          <td>{{ c.post_id }}</td>
          <td>{{ c.user_id }}</td>
          <td><div class="content">{{ c.content }}</div></td>
          <td>
            <button class="btn danger" :disabled="loadingRowId === c.id" @click="remove(c)">删除</button>
          </td>
        </tr>
      </tbody>
      </table>
...
const buildQuery = () => {
  ...
  const postId = parseOptionalInt(postIdRaw.value);
  if (postId === null) throw new Error('帖子ID 请输入正整数');
  if (postId !== undefined) params.set('post_id', String(postId));
...
const fetchComments = async () => {
  ...
  try {
    const qs = buildQuery();
    comments.value = await api<AdminComment[]>(`/admin/comments?${qs}`);
  } catch (err: any) {
...
const remove = async (c: AdminComment) => {
  const ok = window.confirm(`确认删除评论 ${c.id}？`);
  if (!ok) return;
  loadingRowId.value = c.id;
  try {
